...
| Clarity & Presentation | 4.8 | 30-second hero: Network Health separate from Ecosystem Activity. This live run: **HEALTHY** + **SURGE**. DEX line is `24h $2.95B · 1d −2% · vs-7d-ago +60%` (Llama `change_7d` is 24h vs 24h from 7d ago, not 7d-total). Full REV reads **incomplete**. Multipliers are live `?network=Solana`. | Headline data confidence is MIXED when full REV is incomplete or xStocks mcap is a subset (HTTP 200 ≠ HIGH). JSON version lives under `meta`. Charts still mix a short local tape with upstream 30d/90d — labeled, but easy to misread. |
